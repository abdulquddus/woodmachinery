...
if(isset($_GET['page']))
{
 ...
 if($_GET['paging']=='all')
{
      $sql="SELECT * FROM `equipment`  e , eq_images img WHERE e.status=1 and e.id=img.id_equipment and img.is_main=1 and product_type=0 $where $order";
$page=1000;
} else {
  $sql="SELECT * FROM `equipment`  e , eq_images img WHERE e.status=1 and e.id=img.id_equipment and img.is_main=1 and product_type=0 $where $order limit  $start, $page";
}
...
$sqlcount="SELECT count(*) as total FROM `equipment`  e , eq_images img WHERE e.status=1 and e.id=img.id_equipment and img.is_main=1 and product_type=0 $where $order ";
